...
async function scrapeProjects() {
  try {
    ...
    for (let i = 0; i < allProjects.length; i++) {
      ...
      if (!project) {
        console.log(`  ❌ Could not fetch full details for ${projectSummary.slug}`);
        continue;
      }
      ...
      if (project.screenshots?.data && Array.isArray(project.screenshots.data)) {
        for (const screenshot of project.screenshots.data) {
          if (screenshot.attributes?.url) {
            ...
            if (screenshotFilename) {
              const result = await downloadImage(screenshotUrl, screenshotFilename, imagesDir);
              if (result) {
      ...
      if (project.description_blocks && Array.isArray(project.description_blocks)) {
      ...
      if (project.start_date) markdownContent += `start_date: "${project.start_date}"\n`;
      if (project.completed_date) markdownContent += `completed_date: "${project.completed_date}"\n`;
      markdownContent += `active_development: ${project.active_development || false}\n`;
      if (project.short_description) markdownContent += `short_description: "${project.short_description}"\n`;
      if (project.google_play_url) markdownContent += `google_play_url: "${project.google_play_url}"\n`;
      if (project.apple_store_url) markdownContent += `apple_store_url: "${project.apple_store_url}"\n`;
      if (project.github_url) markdownContent += `github_url: "${project.github_url}"\n`;
      if (project.project_url) markdownContent += `project_url: "${project.project_url}"\n`;
      ...
      if (project.screenshots?.data && Array.isArray(project.screenshots.data) && project.screenshots.data.length > 0) {
      ...
      if (project.description_blocks && Array.isArray(project.description_blocks)) {
        const contentMarkdown = processContentBlocks(project.description_blocks, downloadedImages, './images/');
      ...
      if (project.privacy_policy) {
        ...
        if (project.privacy_policy && Array.isArray(project.privacy_policy)) {
          const privacyMarkdown = processContentBlocks(project.privacy_policy, downloadedImages, './images/');
      ...
      if (project.data_delete) {
        ...
        if (project.data_delete && Array.isArray(project.data_delete)) {
          const dataDeleteMarkdown = processContentBlocks(project.data_delete, downloadedImages, './images/');
